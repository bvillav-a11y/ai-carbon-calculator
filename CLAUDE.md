...
3. **`#calcScreen`** — the calculator, **outcome-first**: Results → **Insights & actions** (`renderInsights()` → `#insights`) → collapsed **"⚙ Adjust parameters"** (`toggleParams`/`#paramsSec`, holds the editable Model/Hardware/Grid cards) → Scenario comparison → collapsed **"Calculation trace"** (`toggleTrace`/`#traceSec`) → Confidence + Assumptions. `recalc()` reads the DOM inputs and renders every result block. The survey goes **straight here** (the old `#inferScreen` was removed); `buildInference()`'s `reasons.*` strings are surfaced as ℹ️ tooltips on the matching calc controls via `setTip()`.
...
- `recalc()` is the render hub: `computeCarbon()` → result cells, ledger trace (`#eqbox`), scenario comparison (neutral magnitude bars, Haiku/Sonnet/Opus), confidence bars, assumptions table, and finally `renderInsights()`.
- `renderInsights(p, r)` (last in `recalc()`) writes the **Insights & actions** card: a personalized model right-size line (selected Claude tier vs the task-complexity-recommended tier from `inferModel`, with real savings via `computeCarbon`), a grid insight (savings vs `GRID_CLEAN_REF` when `p.grid > GRID_HIGH`), the always-on right-size principle, and a static checklist. Quick-run / non-Claude → principle-only fallback.
- **Engagement telemetry:** the collection record (see Data collection in README) also carries `dwell_ms` (results-page time), `opened_params`/`opened_trace`/`opened_advanced`, `clicked_share`, `retook_survey` — set via `markEngagement()` in the toggles/share/retake and a `pagehide`/`visibilitychange` finaliser that always fires. New D1 columns are added by a best-effort `ALTER TABLE` migration in the worker's `ensureTable`.
